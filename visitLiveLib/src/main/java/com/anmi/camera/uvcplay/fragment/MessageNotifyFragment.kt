    ...
    private fun onStatusBarChange() {
        ...
        when (deviceConnected) {
            true -> {
                ...
                mLayoutInflaterView?.findViewById<FrameLayout>(R.id.fl_stream)?.visibility  = VISIBLE
                ...
                mLayoutInflaterView?.findViewById<FrameLayout>(R.id.fl_inject_device)?.visibility  = VISIBLE
        ...
        when (event.success) {
            true -> {
                ...
                mLayoutInflaterView?.let { it ->
                    it.findViewById<FrameLayout>(R.id.fl_stream_warning).visibility = GONE
...
//                Utils.toast("推流断开：${event.errorCode}")
                ...
                mLayoutInflaterView?.let { it ->
                    it.findViewById<FrameLayout>(R.id.fl_stream_warning).visibility = VISIBLE
    ...
    private fun handleConnection(connected: Boolean) {
        when (connected) {
            true -> {
                ...
                rlUsbDisConnectWarning?.visibility = GONE
            }
            false -> {
                MyLog.e("${TAG}设备断开")
...
//                openGlView?.visibility = View.GONE
                rlUsbDisConnectWarning?.visibility = VISIBLE
...
// 1. 创建日志拦截器
    ...
    private fun switchCaseRunningLayout(isRunning: Boolean) {
        if (isRunning) {
            caseRunningLayout?.visibility = VISIBLE
            flEmptyCaseChoose?.visibility = GONE
        } else {
            caseRunningLayout?.visibility = GONE
            flEmptyCaseChoose?.visibility = VISIBLE
        }
    }
    ...
    private fun switchMessageListLayout(hasData: Boolean) {
        if (hasData) {
            ...
            messageEmptyRl?.visibility = GONE
        } else {
            messageFl?.visibility = GONE
            messageEmptyRl?.visibility = VISIBLE
        }
    }
